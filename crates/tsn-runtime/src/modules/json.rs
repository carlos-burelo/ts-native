...
pub fn json_parse(_ctx: &mut dyn tsn_types::Context, args: &[Value]) -> Result<Value, String> {
    match args.first() {
        ...
        None => Err("JSON.parse: missing argument".into()),
    }
}
...
pub fn serde_to_tsn(sv: serde_json::Value) -> Value {
    match sv {
...
pub fn json_stringify(_ctx: &mut dyn tsn_types::Context, args: &[Value]) -> Result<Value, String> {
    ...
    let s = serde_json::to_string(&tsn_to_serde(v)).unwrap_or_else(|_| "null".to_owned());
    Ok(Value::Str(Arc::from(s)))
}

pub fn tsn_to_serde(v: &Value) -> serde_json::Value {
    match v {
