...
pub fn build() -> Value {
    ...
    ns.set_field(
        Arc::from("now"),
        Value::NativeFn(Box::new((time_now as NativeFn, "now"))),
    );
    ns.set_field(
        Arc::from("millis"),
        Value::NativeFn(Box::new((time_now as NativeFn, "millis"))),
    );
    ns.set_field(
        Arc::from("toISOString"),
        Value::NativeFn(Box::new((time_to_iso as NativeFn, "toISOString"))),
    );
    ns.set_field(
        Arc::from("fromISO"),
        Value::NativeFn(Box::new((time_from_iso as NativeFn, "fromISO"))),
    );
    ns.set_field(
        Arc::from("toParts"),
        Value::NativeFn(Box::new((time_to_parts as NativeFn, "toParts"))),
    );
    ns.set_field(
        Arc::from("fromParts"),
        Value::NativeFn(Box::new((time_from_parts as NativeFn, "fromParts"))),
    );
