...
pub fn build() -> Value {
    ...
    ns.set_field(
        Arc::from("log"),
        Value::NativeFn(Box::new((console_log as NativeFn, "log"))),
    );
    ns.set_field(
        Arc::from("debug"),
        Value::NativeFn(Box::new((console_debug as NativeFn, "debug"))),
    );
    ns.set_field(
        Arc::from("warn"),
        Value::NativeFn(Box::new((console_warn as NativeFn, "warn"))),
    );
    ns.set_field(
        Arc::from("error"),
        Value::NativeFn(Box::new((console_error as NativeFn, "error"))),
    );
    ns.set_field(
        Arc::from("info"),
        Value::NativeFn(Box::new((console_log as NativeFn, "info"))),
    );
