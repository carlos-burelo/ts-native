...
pub fn net_basic_auth(_ctx: &mut dyn tsn_types::Context, args: &[Value]) -> Result<Value, String> {
    ...
    Ok(Value::Str(Arc::from(format!(
        "Basic {}",
        STANDARD.encode(format!("{}:{}", user, pass).as_bytes())
    ))))
}
...
pub fn build() -> Value {
    ...
    ns.set_field(
        Arc::from("isIp"),
        Value::NativeFn(Box::new((net_is_ip as NativeFn, "isIp"))),
    );
    ns.set_field(
        Arc::from("isIpv4"),
        Value::NativeFn(Box::new((net_is_ipv4 as NativeFn, "isIpv4"))),
    );
    ns.set_field(
        Arc::from("isIpv6"),
        Value::NativeFn(Box::new((net_is_ipv6 as NativeFn, "isIpv6"))),
    );
    ns.set_field(
        Arc::from("joinHostPort"),
        Value::NativeFn(Box::new((net_join_host_port as NativeFn, "joinHostPort"))),
    );
    ns.set_field(
        Arc::from("splitHostPort"),
        Value::NativeFn(Box::new((net_split_host_port as NativeFn, "splitHostPort"))),
    );
    ns.set_field(
        Arc::from("parseURL"),
        Value::NativeFn(Box::new((net_parse_url as NativeFn, "parseURL"))),
    );
    ns.set_field(
        Arc::from("resolveURL"),
        Value::NativeFn(Box::new((net_resolve_url as NativeFn, "resolveURL"))),
    );
    ns.set_field(
        Arc::from("parseQuery"),
        Value::NativeFn(Box::new((net_parse_query as NativeFn, "parseQuery"))),
    );
    ns.set_field(
        Arc::from("buildQuery"),
        Value::NativeFn(Box::new((net_build_query as NativeFn, "buildQuery"))),
    );
    ns.set_field(
        Arc::from("appendQuery"),
        Value::NativeFn(Box::new((net_append_query as NativeFn, "appendQuery"))),
    );
    ns.set_field(
        Arc::from("encodeURI"),
        Value::NativeFn(Box::new((net_encode_uri as NativeFn, "encodeURI"))),
    );
    ns.set_field(
        Arc::from("decodeURI"),
        Value::NativeFn(Box::new((net_decode_uri as NativeFn, "decodeURI"))),
    );
    ns.set_field(
        Arc::from("basicAuth"),
        Value::NativeFn(Box::new((net_basic_auth as NativeFn, "basicAuth"))),
    );
