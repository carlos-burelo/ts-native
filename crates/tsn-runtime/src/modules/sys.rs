...
pub fn sys_env(_ctx: &mut dyn tsn_types::Context, args: &[Value]) -> Result<Value, String> {
    ...
    Ok(std::env::var(key).map(|v| Value::Str(Arc::from(v))).unwrap_or(Value::Null))
}
...
pub fn build() -> Value {
    ...
    ns.set_field(Arc::from("platform"), Value::NativeFn(Box::new((sys_platform as NativeFn, "platform"))));
    ns.set_field(Arc::from("cwd"),      Value::NativeFn(Box::new((sys_cwd      as NativeFn, "cwd"))));
    ns.set_field(Arc::from("args"),     Value::NativeFn(Box::new((sys_args     as NativeFn, "args"))));
    ns.set_field(Arc::from("exit"),     Value::NativeFn(Box::new((sys_exit     as NativeFn, "exit"))));
    ns.set_field(Arc::from("env"),      Value::NativeFn(Box::new((sys_env      as NativeFn, "env"))));
    ns.set_field(Arc::from("setEnv"),   Value::NativeFn(Box::new((sys_set_env  as NativeFn, "setEnv"))));
