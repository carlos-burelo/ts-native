...
type RouteEntry = (String, String, Value); // (method, pattern, callback)
...
fn routes_map() -> &'static Mutex<HashMap<i64, Vec<RouteEntry>>> {
    SERVER_ROUTES.get_or_init(|| Mutex::new(HashMap::new()))
}

// ─── Pending response state (main thread only) ────────────────────────────────
...
fn extract_obj_pairs(v: Option<&Value>) -> Vec<(String, String)> {
    match v {
...
fn extract_params(pattern: &str, path: &str) -> Value {
    let mut obj = ObjData::new();
...
fn build_headers_obj(headers: &[tiny_http::Header]) -> Value {
    let mut obj = ObjData::new();
    for h in headers {
